...
# updated 05/12/2021							    #
# edited by Michael Saul 10/23/2022                     #
#################################################################################
#                                                                               #
#                                                                               #
...
# Michael: Changed the first line to the correct Client IP address for the source field.
$IPTABLES -A INPUT -p TCP --dport 80 -i $Client_NET_IFACE -s 10.0.2.4 -j ACCEPT
# Michael: Wrote this additional line to accpet ICMP traffic from the Client if it was meant for Google's 
#          IP address specifically. Otherwise, the packet would be dropped in the case that the Client attempted
#          to ping anything else or that Gateway attempted to ping anythin (as the source field wouldn't match).
$IPTABLES -A INPUT -p icmp -i $Client_NET_IFACE -s 10.0.2.4 -d 8.8.8.8 -j ACCEPT
...
# Michael: Originally changed "-j DROP" to "-j ACCEPT" when testing, but found it an unncessary addition
#          after establishing the INPUT rules above for this lab.
...
# Michael: Changed "-i" to "-o" and the desitination to match the client
$IPTABLES -A OUTPUT -p TCP --sport 80 -o $Client_NET_IFACE -d 10.0.2.4 -j ACCEPT
# Michael: Added new line to include icmp protocols that are destined for the Client machine.
#          The forward protocol should otherwise deal with any pings that the Server attempts to do,
#          preventing it's relevancy to this OUTPUT rule
$IPTABLES -A OUTPUT -p icmp -o $Client_NET_IFACE -d 10.0.2.4 -j ACCEPT
...
#Michael: edited below line to specify icmp protocol
